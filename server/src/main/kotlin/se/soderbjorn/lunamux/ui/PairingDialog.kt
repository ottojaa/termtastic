...
private val log = LoggerFactory.getLogger("se.soderbjorn.lunamux.ui.PairingDialog")
 ...
 * TTL in between.
 *
 * Called from [SettingsDialog]'s Connections section while its "Pair via
 * QR code" state flag is set — only reachable with allow-remote on, since
 * the button that sets that flag is not rendered otherwise.
 *
 ...
 *   have added a trusted device while the dialog was up).
 */
@Composable
...
private fun buildPairingData(token: String, fingerprint: String?, port: Int): PairingData {
    ...
    // go on. LocalAddresses orders most-reachable first, so the survivors are
    // the ones that matter.
    val emitted = uri.substringAfter("&h=").substringBefore("&").split(',').size
    if (emitted < addresses.size) {
        log.info(
            "PairingDialog: QR carries {} of {} local addresses (dropped {} to stay scannable)",
            emitted, addresses.size, addresses.size - emitted,
        )
    }
...
private fun ColumnScope.PairingContent(data: PairingData, fingerprint: String, port: Int) {
    Text(
        "Scan with the Lunamux app on your phone: Hosts → Scan pairing code.",
        fontSize = 13.sp,
    )
