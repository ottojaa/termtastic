 ...
 *
 * A Termtastic server usually has several addresses (multiple LAN interfaces
 ...
 *
 * Lives in `commonMain` next to [TermtasticClient]; the platform connection
 * holders remain the owners of connection lifecycle — this object only
 * *establishes* connections.
 *
 * @see TermtasticClient
 * @see se.soderbjorn.termtastic.HostPort
 */
 ...
 *
 * @property client the connected [TermtasticClient]; close on failure/teardown.
 ...
 * Android `ConnectionHolder.connectMulti` path (and iOS later), and the home
 * of the pin-mismatch cause-chain scan that used to live in the Android
 * connection holder.
 */
object CandidateConnector {
     ...
     *
     * Each attempt builds a fresh [TermtasticClient] (same [authToken],
     ...
     *   [se.soderbjorn.termtastic.client.storage.LocalRepository.getOrCreateAuthToken]).
     ...
     *   otherwise the last candidate's failure.
     * @see isPinMismatch
     */
    suspend fun connectFirstReachable(
        ...
        for (endpoint in endpoints) {
            ...
            try {
                ...
                // uniform observedMismatch signal the client publishes.
                if (isPinMismatch(t) || client.observedMismatch.value != null) {
                    pinMismatch = t
                } else {
                    lastFailure = t
                }
    ...
    private suspend fun closeQuietly(socket: WindowSocket, client: TermtasticClient) {
