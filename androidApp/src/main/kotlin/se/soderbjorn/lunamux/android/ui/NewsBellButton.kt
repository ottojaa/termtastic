 ...
 *
 * Shared by the Servers and Sessions collapsing top bars (see [HostsScreen] /
 ...
 * the theme accent, matching the other toolbar icons, and its tint pulses
 * continuously when there is actual news so it reads as a call-to-action,
 * mirroring the desktop and iOS bells. When only a version update is available
 * (no news) the bell shows coloured but static.
 *
 * @see HostsScreen
 * @see TreeScreen
 */
 ...
 * tint between the full accent colour and a dimmed variant when there is news.
 *
 * Called from the Servers/Sessions top-bar `actions` slots — always shown, so the
...
fun NewsBellButton(onClick: () -> Unit, shouldPulse: Boolean = true, muted: Boolean = false) {
    ...
    val pulsedTint by transition.animateColor(
        initialValue = SidebarAccent,
        targetValue = SidebarAccent.copy(alpha = 0.3f),
