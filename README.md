...
* On the desktop, open **Settings → Connections → Pair a device** to show a QR code, then scan it from the Android app's Hosts screen (or with the system camera — the code is a `termtastic://pair` link). The phone connects immediately: no addresses to type and no approval dialog to click.
